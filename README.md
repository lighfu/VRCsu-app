<div align="center" width="100%">
  <img src="images/95_20250426102656.png" width="40%">
</div>

<h1 align="center">VRCsu-app</h1>

<br/>
<br/>

<div align="center">VRChatに関わる様々なユーティリティーを利用できるAndroidアプリケーション</div>

<br/>

<div align="center">For every vrchatters</div>

<br/>
<br/>
<div align="center">
  <a href="README_en.md">English (en)</a> •
  <a href="README_ko.md">한국어 (ko)</a> •
  <a href="README_zh.md">中文 (zh)</a> •
  <a href="README_es.md">Español (es)</a> •
  <a href="README_fr.md">Français (fr)</a> •
  <a href="README_ru.md">Русский (ru)</a> •
  <a href="README_pt.md">Português (pt)</a> •
  <a href="README_it.md">Italiano (it)</a> •
  <a href="README_de.md">Deutsch (de)</a>
</div>
<br/>
<br/>

> [!WARNING]
> 現在のリリースはChiffonビルドのみ(先行評価版)を含み、動作が安定しないことがあります。


> [!CAUTION]
> アプリをアンインストールする前に 「概要（このアプリについて）」 から 「Open debug activity」 を押して 「ログアウト」 してください。そうしないとセッションがしばらく残り続けます。
>
>セッションが残り続けるとセッション数の上限に達して、アカウントにしばらくの間ログインできなくなる恐れがあります。

<br/>

<div align="center">
    <a href="https://www.notion.so/1ca5047911ed8011b3d3c13c2d94e775?pvs=4">開発進行状況 / Todoリスト</a>
</div>

---



<div align="center">
  <img src="images/2025_04_26_20.10.42.jpg" height="256px">
  <img src="images/2025_04_26_20.10.53.jpg" height="256px">
  <img src="images/2025_04_26_14.45.54.jpg" height="256px">
  <img src="images/2025_04_26_19.51.19.jpg" height="256px">
  <img src="images/2025_04_26_19.51.27.jpg" height="256px">
</div>

<br/>
<br/>

## インストール

<br/>

このレポジトリのRelease から最新バージョンのAPKをダウンロードしてインストールしてください。かならずGitHubからアプリをダウンロードしてください。このアプリケーションは現在GitHubでのみ提供されています。

<br/>

<div align="center" width="100%">
  <a href="https://github.com/lighfu/VRCsu-app/releases/latest/">
    <img src="images/badge_download.png" width="32%">
  </a>
</div>


<br/>

## ビルド

<br/>

このアプリケーションは現在クローズドソースコードであり、オープンソースではありません。

ビルドは別々に署名されたAPKであり、GooglePlay版とGithub版で混同することできません。

<br/>

- `chiffon` ビルドは不安定なビルド`(Pre Aplha)`です。
予期しない動作をする可能性が大いにあります。
試験評価版に興味がある方はお試しください！

- `stable` は安定リリースになります。
Githubでは `stable` を使用することを強くオススメします。


## バージョン

<br/>

* `v0.1.x-chiffon`: 最初の先行評価リリース

    - 内部的にデータベースが構築されるようになりました
    - 自身の情報を簡易的に編集できるようになりました
    - プライバシーポリシーと利用規約を厳格化しました
    - 試験的にフレンド一覧を実装しました

* `v0.2.x-chiffon`: STTの追加と大規模なローカライズ、その他調整

    - STT機能の追加 (大目玉機能の一つ)
    - ローカライズを徹底的に行いました
    - 10カ国の言語に対応しました
    - モバイルデータ節約のオプション追加
    - アップデーターが大きく進化しました
    - 「私のプロフィール」のレイアウトの微調整
    - 見えない努力をしました。
